<template>
  <router-link :to="{name: 'product',  params: {code: product.code}}"
>
  <div class="card card-size">
	<div class="card__box">
	  <div class="card__img ">
		<span v-if="product.discount > 0" class="card__img-off">{{ product.discount }}% OFF</span>
		<v-img :src="product.image"
		/>
		<div class="card__img-icons">
		  ...
		  <svg width="32" height="32" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path
	...
	<div class="card__text">
	  <div>
		<span class="d-block black">{{ product.name }}</span>
		<div class="card__text-price d-flex">
		  <span class="d-block green card-size-price">${{ product.discount > 0 ? (product.price - (product.price * (product.discount / 100))).toFixed(2) : product.price.toFixed(2) }}</span>
		  <span v-if="product.discount > 0" class="card__text-price-off d-block ml-3 light-grey">${{ product.price.toFixed(2) }}</span>
		</div>
	  </div>
	</div>
  </div>
